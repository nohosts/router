...
const { getWorkers, isFinished } = require('./util');
...
class Router {
  ...
  async proxy(req, res) {
    ...
    if (!workers.totalLen || isFinished(req)) {
      if (!res) {
        throw new Error(workers.totalLen ? 'request is finished.' : 'not found nohost server.');
      }
      req.destroy();
      return;
    }
